...
class Window(QMainWindow, Ui_MainWindow):
    ...
    def get_vid_save_file(self):
        dialog = QFileDialog()
        name = QFileDialog.getSaveFileName(self, 'Save File',str(self.info['title']),'.'+self.combo_video_formats.currentText())
        print(name)
        return name#.split('.')[0]
    
    def get_audio_save_file(self):
        dialog = QFileDialog()
        name = QFileDialog.getSaveFileName(self, 'Save File',str(self.info['title']),'.'+self.combo_audio_formats.currentText())
        print(name)
        return name#.split('.')[0]
